...
sumInc <-
  sumInc %>%
  mutate(exchangrate = ifelse(c_alphan3 %in% c("AUT", "BEL", "BRG", "HRV", "CYP", "CZE", 
                                                "DNK", "EST", "FIN", "FRA", "DEU", "GRC",
                                                "HUN", "IRL", "ITA", "LVA", "LTU", "LUX",
                                                "MLT", "NLD", "POL", "PRT", "ROU", "SVK",
                                                "SVN", "ESP", "SWE"), 0.9016590, exchangrate))
```
...
```{r}
IncComp %>% 
  ggplot(aes(x = reorder(c_alphan3, pppinc), y = pppinc)) + 
  ...
  theme_bw() + 
  labs(title = "Personally Reported Individual Income By PERIOD", #Samantha can you check what period this is over. I don't know what they report it as being, but annual would be scary.  
