...
We need to filter out Venezuela and Taiwan because of missing data we need (the PPP rates for conversion of income)
```{r filter_out_VE_TW}
sumInc <- 
  sumInc %>% 
...
PPPs <- 
  PPPcodes %>% 
  filter(Year == 2015 & TRANSACT == "PPPP41") %>% 
  filter(country %in% sumInc$country)
```
...
Inc1 <- 
  narInc %>% 
  mutate(incstring = as.character(value)) %>%
  filter(str_detect(value, "999"))
```
